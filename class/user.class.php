...
class user
{
	...
	public function login(array $data)
	{
		...
		if ($sth->fetchColumn()) {
			...
			if ($user and $this->checkPassword($data['password'], $user['password'])) {
				...
				static::removeSessionCode($data['session_code']);
				throw new Exception(trans('Username or password are incorrect'));
			}
		} else {
	...
	public function register(array $data)
	{
		...
		if (isset ($error)) {
			...
			if (mail::send('register', $data['email'], ['activation_code' => $activation_code, 'password' => $data['password'], 'username' => $data['username'], 'email' => $data['email']])) {
				...
				$sth = $db->prepare('INSERT INTO `' . _DB_PREFIX_ . 'user`(`username`, `email`, `password`, `activation_code`, `register_ip`) VALUES (:username,:email,:password,:activation_code,:register_ip)');
				$sth->bindValue(':username', $data['username'], PDO::PARAM_STR);
				$sth->bindValue(':email', $data['email'], PDO::PARAM_STR);
	...
	public function completeData(string $code, array $data)
	{
		...
		$sth = $db->prepare('UPDATE ' . _DB_PREFIX_ . 'user SET username=:username WHERE active=1 AND activation_code=:code AND username="" LIMIT 1');
		$sth->bindValue(':username', $data['username'], PDO::PARAM_STR);
		$sth->bindValue(':code', $code, PDO::PARAM_STR);
		$sth->execute();
	...
	public static function listAll()
	{
		global $db;
		$users = [];
		$sth = $db->query('SELECT * FROM ' . _DB_PREFIX_ . 'user ORDER BY username');
		foreach ($sth as $row) {
			$users[] = $row;
		}
		return $users;
