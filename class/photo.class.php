...
class photo {
	...
	public static function add(){
		...
		if(!in_array($path_parts['extension'] , ['jpg','jpeg','png'])){
			...
			return ['status'=>0, 'info'=>trans('Photo limit exceeded').' ('.$settings['photo_max'].')'];
		}else{
			...
			imagecopyresampled($tmp,$src,0,0,$x,$y+50,$newwidth,$newheight,$smallestSide,$smallestSide);
			...
			return ['status'=>1, 'id'=>$id, 'url'=>$folder.$url, 'thumb'=>$folder.$thumb, 'remove_title'=>trans('Delete photo').': '.$url, 'Delete'=>trans('Delete')];
		}
	}
