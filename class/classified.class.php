...
class classified {
	...
	public static function list(int $limit=10,array $data=[],string $type='all'){
				...
				if(!empty($options)){
					...
					foreach($options as $key => $value){
						if(is_array($value)){
							...
							$where_statement .= ' (SELECT count(1) FROM '._DB_PREFIX_.'option_value ov, `'._DB_PREFIX_.'option` op WHERE op.id=ov.option_id AND ov.option_id=:option_id_'.$i.'_key AND ov.classified_id=c.id AND ov.value LIKE :option_id_'.$i.'_value LIMIT 1) > 0 ';
							$bind_values['option_id_'.$i.'_key'] = $key;
		...
		if(!empty($data['sort'])){
			if($data['sort']=='random'){
				...
				$sort = ' c.price_free DESC, c.price=0, c.price, c.date_start DESC ';
			}elseif($data['sort']=='most_expensive'){
				$sort = ' c.price DESC, c.date_start DESC ';
		...
		}elseif($type=='classifieds' or $type=='my_classifieds' or $type=='clipboard'){
		...
		if($type!='admin'){
			if($sort == ' o.date_start desc '){
				$sort = ' if(c.promoted="1", c.promoted_date_start, coalesce(c.promoted,0)) DESC, '.$sort;
			}else{
	...
	public static function add(array $data){
		...
		if($active){
			$sth = $db->prepare('UPDATE `'._DB_PREFIX_.'classified` SET `active`=1 WHERE id=:id LIMIT 1');
		...
		if($user->getId()){
			if($send_mail){
				mail::send('classified_start',$data['email'],['classified_name'=>$data['name'], 'classified_url'=>path('classified',$id,$classified['slug']),'user_id'=>$user->getId()]);
				static::refreshCountCategories($id);
			}
		}else{
			if($send_mail){
				mail::send('classified_start_not_logged',$data['email'],[
					'classified_edit_link'=>path('edit',$id,$classified['slug']).'?code='.$code, 'classified_activate_link'=>path('classified',$id,$classified['slug']).'?activate&code='.$code,
					'classified_name'=>$data['name'],
					'classified_url'=>path('classified',$id,$classified['slug'])]);
			}
		}
	...
	public static function edit(int $id,array $data,bool $checkCost=false){
		...
		$sth->bindValue(':youtube_url', trim(strip_tags($data['youtube_url'])), PDO::PARAM_STR);
	...
	public static function show(int $id,string $controller=''){
		...
		if($classified){
			...
			if($controller=='edit' || $controller=='add_similar'){
				...
				$sth = $db->prepare('SELECT op.name, ov.value FROM '._DB_PREFIX_.'option_value ov, `'._DB_PREFIX_.'option` op WHERE ov.option_id = op.id AND ov.classified_id=:classified_id ORDER BY op.position');
				$sth->bindValue(':classified_id', $id, PDO::PARAM_INT);
				$sth->execute();
				while ($row = $sth->fetch(PDO::FETCH_ASSOC)){
