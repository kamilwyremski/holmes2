...
class payment {
  ...
  public static function new(string $company,int $item_id,string $type){
  	...
  	if(!empty($classified)){
  		...
  		$sth->execute();
  		$id = $db->lastInsertId();
  		$url = path('classified',$item_id,$slug).'?';
  		...
  		if($payment['amount']/100<=$amount){
  			if($payment['type']=='promote'){
  				...
  				if(!$classified['promoted']){
  					mail::send('start_promote',$classified['email'],['classified_name'=>$classified['name'], 'classified_url'=>path('classified',$classified['id'],$classified['slug'])]);
  				}
  				...
  				if(!$classified['active']){
  					if($classified['user_id']){
  						mail::send('classified_start',$classified['email'],['classified_name'=>$classified['name'], 'classified_url'=>path('classified',$classified['id'],$classified['slug'])]);
  					}else{
  						mail::send('classified_start_not_logged',$classified['email'],['classified_edit_link'=>path('edit',$classified['id'],$classified['slug']).'?code='.$classified['code'], 'classified_activate_link'=>path('classified',$classified['id'],$classified['slug']).'?activate&code='.$classified['code'], 'classified_name'=>$classified['name'], 'classified_url'=>path('classified',$classified['id'],$classified['slug'])]);
  					}
