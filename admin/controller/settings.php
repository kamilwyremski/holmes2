	...
	if(!_ADMIN_TEST_MODE_ and isset($_POST['action'])){
		if($_POST['action']=='save_settings' and !empty($_POST['base_url']) and !empty($_POST['email']) and !empty($_POST['title']) and checkToken('admin_save_settings')){
			...
			settings::saveArrays(
				...
				['add_classifieds_not_logged','automatically_activate_classifieds','enable_articles','rss','generate_sitemap','check_ip_user','required_type','required_category','required_subcategory','required_phone','required_address','required_state','required_price','google_maps','show_similar_classifieds','photo_add','watermark_add','hide_data_not_logged','hide_phone','hide_email','hide_views','mail_attachment','smtp']
			);
