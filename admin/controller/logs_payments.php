...
if($admin->is_logged()){
	...
	if(!_ADMIN_TEST_MODE_ and isset($_POST['action'])){
		if($_POST['action']=='remove_logs' and checkToken('admin_remove_logs')){
			if($_POST['type']=='only_removed'){
				payment::removeLogsWithoutClassifieds();
				header('Location: ?controller=logs_payments');
				die('redirect');
	...
	$render_variables['payments'] = payment::listLogs(100);

	$title = trans('Logs payments').' - '.$title_default;
}
