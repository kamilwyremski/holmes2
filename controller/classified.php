...
if(isset($_GET['id']) and $_GET['id']>0 and classified::checkActive($_GET['id'],$code)){
	...
	if($settings['show_contact_form_classified'] and isset($_POST['action']) and $_POST['action']=='send_message' and !empty($_POST['name']) and (!empty($_POST['email']) or $user->getId()) and !empty($_POST['message']) and (isset($_POST['captcha']) or isset($_POST['g-recaptcha-response'])) and (isset($_POST['rules']) or $user->getId())){
		...
		if(isset($error)){
			...
			$classified = classified::show($_GET['id']);
			if(mail::send('report_abuse',$settings['email'],['email'=>$email, 'message'=>$_POST['message'], 'classified_name'=>$classified['name'], 'classified_url'=>path('classified',$classified['id'],$classified['slug'])])){
				$render_variables['alert_success'][] = trans('The message was correctly sent');
			}else{
				$render_variables['alert_danger'][] = trans('The message was not sent');
				$render_variables['report_abuse'] = ['email'=>$_POST['email'], 'message'=>$_POST['message']];
	...
	if($classified['youtube_url']){
		$classified['youtube_embed'] = getYoutubeIdFromUrl($classified['youtube_url']);
	}
