...
if (!empty ($classifieds)) {
	foreach ($classifieds as $key => $value) {
		...
		$rssfeed .= '<link>' . path('classified', $value['id'], $value['slug']) . '</link>';
		$rssfeed .= '<guid>' . path('classified', $value['id'], $value['slug']) . '</guid>';
		...
		if ($value['thumb']) {
			$rssfeed .= '&lt;br&gt;&lt;br&gt;&lt;a href="' . path('classified', $value['id'], $value['slug']) . '"&gt;&lt;img src="' . $settings['base_url'] . '/upload/photos/' . $value['thumb'] . '" height="80"/&gt;&lt;/&gt;';
		}
		$rssfeed .= '</description>';
		$rssfeed .= '</item>';
	}
